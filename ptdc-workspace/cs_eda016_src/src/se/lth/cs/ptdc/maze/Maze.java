...
public class Maze {
	...
	private static final String JAR_NAME = "cs_eda016.jar";
	 ...
	 * maze<nbr>.maze, which must be in cs_eda016.jar, which must be on the
	 * class path.
	 * 
	 * @param nbr
	...
	private void create(int nbr) {
		JarInputStream jin = getStream(JAR_NAME);
		if (jin == null) {
			System.err.println("Maze error: could not find " + JAR_NAME);
			System.exit(1);
		}
		...
		if (entry == null) {
			System.err.println("Maze error: could not find " + mazeFileName
					+ " in " + JAR_NAME);
			System.exit(1);
		}
		...
		try {
			...
			System.err.println("Maze error: cannot read " + mazeFileName);
	...
	private JarInputStream getStream(String jarFileName) {
		 ...
		 */
		String fullpath = (System.getProperty("java.class.path") + ":" + System
				.getProperty("java.library.path"));
		Scanner sc = new Scanner(fullpath).useDelimiter(":");
		File f = null;
		while (f == null && sc.hasNext()) {
			String name = sc.next();
			if (name.endsWith(JAR_NAME)) {
				f = new File(name);
			} else if (!name.endsWith(".jar")) {
				// name is a directory, hopefully
				f = new File(name + "/" + JAR_NAME);
			}
			if (f != null && !f.exists())
				f = null;
		}
